export function validateJuso(value){
	value =value.replace(/(^\s*)|(\s*$)/g, ""); //앞뒤 공백 제거
	value =value.replace(/\s{2,}/gi," "); //2칸이상 공백 1칸으로 변경
...
export function checkValidate1(keyword){	
	if (keyword.trim() == "") {
		alert('검색어를 입력해주세요!');
		return false;
	}
...
export function checkValidate2(keyword){
	//시도
...
export function keyWordMin(keyword){
	if (keyword.length <= 1){
...
export function keyWordMax(keyword){
	if (keyword.length > 80){
		alert("검색어가 너무 깁니다. (한글40자, 영문,숫자 80자 이하)");
		return false;
	} else {
		return true;
	}
...
export function keyWordIsNumber(keyword){
	if (keyword.replace(/[0-9]/g,"").replace( / /g,"").length == 0 ) {
		...
		alert("검색어는 문자와 숫자가 같이 입력되어야 합니다.");
		return false;
...
export function keyWordIsMaxNumber(keyword){
	//숫자만 추출함.
	var numStr = keyword.replace(/[^0-9]/g," ").replace(/ +/g," ");
	//숫자들의 배열
	var numStrArr = numStr.split(" ");
	
	for (var i=0; i<numStrArr.length;i++){
		//숫자길이가 10자리 초과
		if (numStrArr[i].length > 10){
			alert("검색어에 너무 긴 숫자가 포함되어 있습니다. (숫자 10자 이하)");	
			return false;
		}		
	}
	return true;
...
export function regExpCheckJuso(strKeyword){
	...
	if(reqExp1.test(strKeyword))
	{
		...
		for(var i=tempLength-1;i>=0;i--)
		{
			...
			{
				//숫자 앞의 단어가 공백,영문이 아닐때(한글일 경우)
				if(charKeyword != " " && !checkEng.test(charKeyword))
				{
					//지하, 산 등의 제외 단어가 아닐 경우 해당 문자와 숫자사이에 공백 추가
					if(tempKeyword.charAt(i-1) + charKeyword != '지하' && charKeyword != '산'){
...
export function insertString(key,index,string){
	if(index >0)
...
export function checkValidate3(keyword){
