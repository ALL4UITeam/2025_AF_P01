<template>
	<CommonBreadcrumb />
    <HeaderTitle title="홍보 자료실" />

    <div class="tableHeaderForm">
        <TabMenu :model="tabItems" :activeIndex="0" class="tabList">
        ...
        <div class="tableHeaderFormRight">
			<CustomDropdown :options="searchItemOptions" optionLabel="label" optionValue="value" v-model="inqCndParams.ntcTypeCd" title="구분" placeholder="전체"/>
            <CustomDropdown :options="items" optionLabel="label" optionValue="value" v-model="inqCndParams.searchTag" title="구분" placeholder="전체"/>
    ...
    <div class="dataTable__wrap">
		<DataTable :value="searchDataList" ref="dataTableRef" :loading="lazyLoading">
            ...
            <Column field="pstTtl" header="제목" class="alignLeft">
                <template #body="slotProps">
                    <div class="linkHover">
                        <a href="#" @click.prevent="fnDtlView(slotProps.data.prRscntrMgno)" class="alignLink">
            ...
            <Column style="width:20%;" field="srcNm" header="출처">
                <template #body="slotProps">
                    <span class="centerHover">
                        {{ slotProps.data.srcNm }}
                    </span>
                </template>
            </Column>
...
const fnSrch = async () => {
    ...
    axios.post('/api/ahi/selectReferenceList', inqCndParams)
        .then((response) => {
...
const updateCaption = () => {
    const width = window.innerWidth;
    header.value = width > 900 ? '홍보자료 자료실 리스트로 NO, 제목, 등록일, 조회수,첨부파일 여부를 제공합니다' : '홍보자료 자료실 리스트로 제목, 등록일 정보를 제공합니다';
