<template>
	<CommonBreadcrumb />
	<HeaderTitle title="우편 번호 조회" />

  <form @submit.prevent="AddressObject">
    <fieldset>
      <legend>우편 번호 검색</legend>
      <div class="commonContentSearchForm">
        ...
        <div class="commonContentSearchForm__type">
          <div class="commonContentSearchForm__item02">
            <label for="sbdno">우편번호</label>
            <CustomInputText v-model="inqCndParams.sbdno" class="input" id="sbdno" placeholder="우편번호 입력" @keydown.enter="fnSrch"
              title="우편번호 입력해주세요" type="number" :maxLength="5" />
          </div>
  ...
  <div class="dataTable__wrap">
    ...
    <DataTable class="tableStyle01 noneLine" :value="searchDataList" ref="dataTableRef" :loading="lazyLoading">
      ...
      <Column header="우편번호" field="sbdno">
        <template #body="slotProps">
          <a @click.prevent="fnRoadSectionMap(slotProps.data)" href="#" class="alignLink">
            <span class="text">
              {{ slotProps.data.sbdno }}
              <i class="bi bi-geo-alt-fill" />
            </span>
          </a>
        </template>
      </Column>
      <Column header="시/도" field="lgvReplcCd">
        <template #body="slotProps">
          {{ slotProps.data.kcpNm }}
        </template>
      </Column>
      <Column header="시/군/구" field="lgvReplcCd">
        <template #body="slotProps">
...
const fnSrch = () => {
	...
	axios.post('/api/ahu/selectKarbSbdList', inqCndParams)
		.then((response) => {
...
const apiDtlSrch = () => {
	return new Promise((resolve, reject) => {
		axios.post('/api/ahu/selectKarbSbdListExcel', inqCndParamsOrigin).then(response => {
			resolve(response);
		})
		.catch(error => {
			reject(error);
...
const fnRoadSectionMap = (data) => {
	...
	url += 'searchType=post&sbdno=' + data.sbdno;
...
onMounted(() => {
	...
	if (dataTableEl) {
		...
		caption.textContent = '우편번호 조회 리스트로, 순번, 우편번호, 시/도, 시/군/구를 제공합니다.';
